...
- can call Python EZ PPO code through `reticulate`
...
- reuse your existing EZ_Optimization code
- train the EZ PPO model or load an existing checkpoint
- produce out of sample signals and a cumulative return path

Here we just call a single function `run_ez_pipeline()` that you implement in Python.

```{r python-ez, eval=FALSE}
# This will only work once reticulate is pointed at the correct Python
# and src/ez_runner.py is present and imports correctly.
...
- `signals`: data frame with an index column and `signal_ez`
- `cr_ez`: a vector of cumulative returns for the EZ strategy

You can left join this onto `Trading_Strategy` after you align the index with `predicting_date`, then produce comparison plots for RL vs regression vs buy and hold.
